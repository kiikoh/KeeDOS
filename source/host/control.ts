...
module TSOS {
  export class Control {
    ...
    public static updateMemory(address: number, value: number) {
      ...
      (<HTMLDataElement>(
        document.querySelector(
          `#memoryDisplay > tr:nth-child(${row}) > td:nth-child(${col})`
        )
      )).innerText = Utils.toHexString(value, 2);
    }

    public static updatePCBs() {
      const pcbTableBody = <HTMLTableSectionElement>(
        document.querySelector("#taskManager > tbody")
      );
      const removeTerminated = (<HTMLInputElement>(
        document.querySelector("#terminatedToggle > input")
      ))!.checked;
    ...
    public static updateCPU() {
      const cpuTableBody = <HTMLTableSectionElement>(
        document.querySelector("#cpuDisplay > tbody")
      );
      const row = document.createElement("tr");
      row.innerHTML = `
    ...
    public static updateDisk() {
      ...
      const diskTableBody = <HTMLDivElement>(
        document.querySelector("#diskDisplay")
      );
            ...
            dataCard.innerHTML = `
                            ...
                            <div class="data-card-body">
                                <div class="data-card-row">
                                    ...
                                    <span class="data-card-value">${data[1]}:${
              data[2]
            }:${data[3]}</span>
                                </div>
                                <div class="data-card-row">
    ...
    // Host Events
    //
    public static async hostBtnStartOS_click(
      ...
      (<HTMLButtonElement>(
        document.getElementById("btnSingleStepToggle")
      )).disabled = false;
      ...
      _taskbarTimeID = setInterval(() => {
        ...
        document.getElementById(
          "time"
        )!.innerText = `Time: ${now.toLocaleTimeString()}`;
      }, 1000);
    ...
    public static hostBtnSingleStepEnable_click(btn: HTMLButtonElement): void {
      const singleStepOnceBtn = <HTMLButtonElement>(
        document.getElementById("btnSingleStepOnce")
      );
    ...
    public static hostBtnToggleDisk_click(btn: HTMLButtonElement): void {
      const diskContainer = <HTMLDivElement>(
