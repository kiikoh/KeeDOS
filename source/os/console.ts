...
module TSOS {
  export class Console {
    ...
    public handleInput(): void {
      while (_KernelInputQueue.getSize() > 0) {
        ...
        if (chr === "Enter" || chr === 13) {
          ...
          // something like that
          const rec =
            recs.length === 1 ? recs[0] : Utils.greatestCommonSubstring(recs);
          ...
          const newBuff =
            _OsShell.history[_OsShell.history.length - this.historyIndex] ?? "";
          ...
          if (_CPU.isExecuting) _Scheduler.killProcess();
    ...
    public setBuffer(text: string) {
      ...
      const promptOffset = _DrawingContext.measureText(
        this.currentFont,
        this.currentFontSize,
        _OsShell.promptStr
      );
    ...
    public putText(text: string): void {
      ...
      if (text !== "") {
        ...
        let offset = _DrawingContext.measureText(
          this.currentFont,
          this.currentFontSize,
          text
        );
          ...
          if (numWords === 0) {
            ...
            while (
              CanvasTextFunctions.measure(
                this.currentFont,
                this.currentFontSize,
                letters.slice(0, numLetters).join("")
              ) +
                this.currentXPosition <
              _Canvas.width
            ) {
    ...
    public advanceLine(): void {
      ...
      if (this.currentYPosition > _Canvas.height) {
        // we need to scroll
        const hist = _DrawingContext.getImageData(
          0,
          0,
          _Canvas.width,
          _Canvas.height
    ...
    public screensaver(): void {
      ...
      if (
        _screensaverState.x > _Canvas.width - width ||
      ...
      if (
        _screensaverState.y > _Canvas.height - height ||
